...
Compare this version of R that is running in the container to the version of R you have on your computer.
...
We can pass in multiple envivironment variables by using multile `-e` flags.
For example, we can set the password for the rstudio user with the `PASSWORD` environment variable,
but we can also disable the login screen with the `DISABLE_AUTH=true` variable.

```bash
docker run --rm -p 8787:8787 -e PASSWORD="apassword" -e DISABLE_AUTH=true rocker/rstudio:4.4.2
```

Each container will have its own set of environment variables to contol its behaviour.
For the `rocker/rsutdio` project,
you can find the environment variable documentation here: <https://rocker-project.org/images/versioned/rstudio.html#environment-variables>.
...
```bash
docker run --rm -p 8888:8888 quay.io/jupyter/datascience-notebook:2025-03-14
```
...
:::{.callout-important}
Do not forget to copy the `token=` portion of the URL,
if you only go to `localhost:8888` or `http://127.0.0.1:8888` it will ask you
...
continuumio/miniconda3:23.9.0-0                   6fbaadd54391        536MB             0B    U
hello-world:latest                                1b44b5a3e06a       10.1kB             0B
postgres:15-alpine                                546a2cf48182        274MB             0B
quay.io/jupyter/datascience-notebook:2025-03-14   1a211fc8a877       6.42GB             0B
rocker/r-ver:4.3.2                                f0b4e058dcfa        849MB             0B
rocker/rstudio:4.3.1                              c8bd7a718854          2GB             0B
rocker/rstudio:4.4.2                              4503182694c4       2.36GB             0B
```
...
In the examples above we have used the `rocker/rstudio` container that is coming from DockerHub,
and the jupyter server comes from `quay.io`.
...
- [quay.io/jupyter/datascience-notebook](https://quay.io/repository/jupyter/datascience-notebook?tab=tags)
...
# uses the jupyterlab from the base environment
