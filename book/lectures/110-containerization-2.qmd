...
Below we demonstrate how to launch and run containers using the [`condaforge/miniforge3` image](https://hub.docker.com/r/condaforge/miniforge3) as an example. We will slowly walk through the 4 basic steps need to run a container:
...
If you only see `container_user/container_image:container_image_version_tag` Docker will assume you are using Docker Hub as the container registry (this is the default).
In the previous chapter we ran the `quay.io/jupyter/datascience-notebook:2025-03-14` container from the `quay.io` registry,
and `rocker/rstudio:4.4.2` from Docker Hub.

Sometimes the `container_image_version_tag` is ommitted, when this happens the most recent container is pulled, and has the version tag `latest`. In data analysis projects we try to avoid using this strategy because Docker is lazy and if you already have the `latest` version on your local host machine, it will not update it when you re-run `docker pull container_user/container_image` even if there is a newer version of the container image on the remote container registry.
...
condaforge/miniforge3:25.3.1-0   322e23c921ba        447MB             0B
```
...
We can do this via: `-v <path_to_local_computer_directory>:<absolute_path_to_container_directory>`
...
This is a useful shorthand command when you work with mutliple people, so the same command will work on different computers since you are not hard specifying the local computer path.

To mount our current directory to a container from the `condaforge/miniforge3:25.3.1-0` image we type the following on your laptop:
...
<!-- TODO: dan: 202526w1 i don't know if we still need this section -->
...
### Use a container to execute a script
