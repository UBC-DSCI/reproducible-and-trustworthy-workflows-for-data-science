...
### Poetry, Hatch, and the evolution of Python packaging
...
This new single configuration file has inspired some new tools to be created in the package building and management ecosystem in Python.

#### Poetry

One of the most recent and simplest to use is Poetry (created in 2018). When used to build packages, Poetry roughly does two things:
...
[Hatch](https://hatch.pypa.io/latest/) is a newer python package manger (first 1.0 release in 2022).
It is [PEP-517](https://peps.python.org/pep-0517/) and [PEP-518](https://peps.python.org/pep-0518/) compliant and can do all the environment management that poetry does.
To avoid convusion, this textbook will not use the environment features of `hatch`, only the package build backend, `hatchling`.
It assumes you are using `conda` for the main python verison and environment management,
all package dependencies will not be placed in the `environment.yml` file, these will all go into the `pyproject.toml` file.

Your python `environment.yml` file will be very minimal, all other packages will be specified in the `pyproject.toml`.
...
:::{.callout-note}
This book and resources is slowly migrating towards the PyOpenSci + Hatch + Hatchling way of building packages.
We will migrate to the official PyOpenSci packaging guide: <https://www.pyopensci.org/python-package-guide/index.html>

All references to `cookiecutter` and `poetry` should look for the corresponding reference in the PyOpenSci tutorial on `copier` and `hatch`.
For documentation references to `sphinx` would go do the `quartodoc` site: <https://machow.github.io/quartodoc>
:::
...
| Templating tools                                      | Cookiecutter / Copier | `usethis`             |
| Documentation building tools                          | sphinx / quartodoc    | pkgdown               |
| Package build tools                                   | Poetry / Hatch        | `devtools`            |
