...
A `conda-lock.yml` file is a file that specifies all the packages, its versions,
...
:::{.callout-important}
When creating an `environment.yml` file,
make sure you **only** have the `conda-forge` channel.
**DO NOT** mix the conda-forge channel with defaults.
It will sometimes cause the solver to look for packages in the wrong location
and the solver will either fail or take an even longer time to solve.

Also, if you are using `conda env` to create the `environment.yml` file,
remember to remove the `prefix` line on the bottome of the file.
This helps with environment file portability.
:::
...
If you are on a ARM-based M-Chip Mac, you will need to generate the `linux-aarch64` platform,
if you want to use `conda-lock` with Docker.
Typically you will end up running this command to generate a lock file for all platforms:
...
See this repository as a reference of `conda-lock.yml` being used in a data anslysis project:
